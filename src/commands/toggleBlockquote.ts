...
// Toggle a `> ` blockquote prefix across the selection as a whole: if every
// relevant line is already a blockquote, remove the prefix from all; otherwise
...
export const toggleBlockquote: Command = (view) => {
  ...
  for (const line of lines) {
    const existing = prefixLen(line.text, BLOCKQUOTE_RE)
    if (allQuotes) {
      changes.push({ from: line.from, to: line.from + existing })
    }
    else if (existing === 0) {
      const replace = otherPrefixLen(line.text, [BULLET_RE, ORDERED_RE])
  ...
  view.dispatch(
    state.update({
      ...
      scrollIntoView: true,
      annotations: Transaction.userEvent.of("input.quote"),
    }),
  )
  return true
}
